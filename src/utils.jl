...
function controller_setup(# Scene Loader parameters
    ...
    sim_param = SimulationParameter(dtc, scene_loader.dto, prediction_steps,
                                    num_samples, cost_param)
    # sim_param = SimulationParameter(dtc, 0.25, prediction_steps,
    #                                 num_samples, cost_param)
    ado_inputs = fetch_ado_positions!(scene_loader, return_full_state=true);
    ado_positions = reduce_to_positions(ado_inputs)
...
function visualize!(color_dict::Dict, w::WorldState,
                    ...
                    figsize=(600, 400), legend=:topright,
                    legendfontsize=8.,
                    xlim=(0., 20.), ylim=(0., 20.), markersize=8.0,
                    show_velocity=true,
    ...
    plot!([p[1] for p in values(target_trajectory)],
          [p[2] for p in values(target_trajectory)],
          color=color_dict[robot_key], label="Target Trajectory", linestyle=:dash, alpha=0.5)
    goal_pos = collect(values(target_trajectory))[end];
    scatter!((goal_pos[1], goal_pos[2]), color=color_dict[robot_key],
             label="Ego Goal",
             markershape=:star5,
             markersize=markersize)
    if !isnothing(nominal_trajectory)
        plot!([p[1] for p in nominal_trajectory],
              [p[2] for p in nominal_trajectory],
              color=color_dict[robot_key], label="Ego Nominal Trajectory",
              alpha=1.0, linestyle=:dashdot, linewidth=1.5)
    end
...
function make_gif(result::Union{EvaluationResult, BICEvaluationResult, CrowdNavEvaluationResult};
    ...
    @assert dtplot >= dtc;
    dtplot = dtplot;
    if typeof(result) == EvaluationResult
