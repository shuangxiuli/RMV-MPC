...
function TrajectronSceneLoader(param::TrajectronSceneParameter;
   ...
   # to manually delete the ado agent from the outputs_dict.
    ...
    @assert in(param.test_data_name, ["eth", "hotel", "univ", "synthetic_eth", "noised_eth", "multimodal_noised_eth", "removed_eth"] .* "_test.pkl");
    file_dir = @__DIR__;
    model_dir = normpath(joinpath(file_dir, "..",
                                  "Trajectron-plus-plus", "experiments", "pedestrians",
                                  "models"));
    if in(param.test_data_name, ["eth_test.pkl", "synthetic_eth_test.pkl", "noised_eth_test.pkl", "multimodal_noised_eth_test.pkl", "removed_eth_test.pkl"])
